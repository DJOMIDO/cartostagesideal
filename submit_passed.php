...
if (empty($level) || empty($type) || empty($sujet) || empty($duree) || empty($lieu) || empty($organisme) || empty($contact) || empty($email) || empty($description) || empty($date)) {
    echo 'Veuillez remplir tous les champs obligatoires !';
    exit;
}

// Prepare SQL statements and execute them
$sql = "INSERT INTO offres (niveau, parcours, sujet, duree, lieu, organisme, contact, email, description, dumas, date, effectue) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)";
$stmt = $db->prepare($sql);
...
$stmt->bindValue(9, $description);
$stmt->bindValue(10, $dumas);
